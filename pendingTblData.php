...
if (!empty($_POST) ) {
    ...
    $query = "SELECT id, da"."te, event_type, package_type, time_start, amount FROM event_reservation WHERE status='pending'";
    ...
    if(!empty($_POST['search']['value'])){
        ...
        $where = "WHERE status='pending' AND (".implode(" OR " , $where).")";// id like '%searchValue%' or name like '%searchValue%' ....
        /* End WHERE */
    ...
    /* END SEARCH */
        ...
        $sql = sprintf("SELECT id, da"."te, event_type, package_type, time_start, amount FROM %s WHERE status='pending' ORDER BY %s %s limit %d , %d", "event_reservation" ,$orderBy,$orderType ,$start , $length);
        $param = ["id", "date", "event_type", "package_type", "time_start", "amount"];
        $data = $_dbCall->getResultsArray($sql, $param);
