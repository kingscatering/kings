...
if (!empty($_POST) ) {
    ...
    $query = "SELECT id, da"."te, event_type, package_type, time_start, amount FROM event_reservation WHERE status='cancelled' AND customer_id = $customer_id";
    ...
    if(!empty($_POST['search']['value'])){
        ...
        $where = "WHERE status='cancelled' AND customer_id = $customer_id AND (".implode(" OR " , $where).")";// id like '%searchValue%' or name like '%searchValue%' ....
        /* End WHERE */

        // $sql = sprintf("SELECT * FROM %s WHERE status='cancelled' ", "event_reservation");//Search query without limit clause (No pagination)
        $param = ["id", "date", "event_type", "package_type", "time_start", "amount"];
    ...
    /* END SEARCH */
        ...
        $sql = sprintf("SELECT id, da"."te, event_type, package_type, time_start, amount FROM %s WHERE status='cancelled' AND customer_id = $customer_id ORDER BY %s %s limit %d , %d", "event_reservation" ,$orderBy,$orderType ,$start , $length);
        $param = ["id", "date", "event_type", "package_type", "time_start", "amount"];
        $data = $_dbCall->getResultsArray($sql, $param);
